...
class ACDC(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, root, split='train', mode='fine', target_type='semantic', transform=None):
        ...
        for weather_condition in os.listdir(self.images_dir):
            img_dir = os.path.join(self.images_dir, weather_condition,split)
            target_dir = os.path.join(self.targets_dir, weather_condition,split)

            for folder in os.listdir(img_dir):
                img_folder_dir = os.path.join(img_dir,folder)
                target_folder_dir =  os.path.join(target_dir,folder)

                for file_name in os.listdir(img_folder_dir):
                    self.images.append(os.path.join(img_folder_dir, file_name))
                    target_name = '{}_{}'.format(file_name.split('_rgb_anon')[0],
                                             self._get_target_suffix(self.mode, self.target_type))
                    self.targets.append(os.path.join(target_folder_dir, target_name))
    ...
    def __getitem__(self, index):
        ...
        #print(os.path.join(self.root,uncertainty_mask_path))
        #print(os.path.exists(os.path.join(self.root,uncertainty_mask_path)))
